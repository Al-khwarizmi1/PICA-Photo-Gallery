...
function maccontroller() {
	...
	@session_start();
	if($_SESSION['app_wp_token'] == '' ){
	$_SESSION['app_wp_token'] = md5(uniqid(AppThA_WP-tOKeN,true));
	}
	$site_url = get_bloginfo('url');
	$folder = dirname(plugin_basename(__FILE__));
...
<script type="text/javascript">
        var site_url,mac_folder,numfiles,upload_token;
        upload_token = '<?php echo md5(DB_NAME); ?>';        
        site_url = '<?php echo $site_url; ?>';
        var url = '<?php echo $site_url; ?>';
        mac_folder  = '<?php echo $folder; ?>';
        keyApps = '<?php echo $configXML->keyApps; ?>';
        videoPage = '<?php echo $meta; ?>';
        var pica_token = '<?php echo $_SESSION['app_wp_token'];?>';
...
QueueCountApptha = 0;
    ...
    if(document.getElementById('mac-test-list'))
               ...
               dragdr("#mac-test-list").sortable({
                ...
                update : function () {
                    var order =dragdr('#mac-test-list').sortable('serialize');
                     var f = dragdr("#info").load(site_url+"/wp-content/plugins/"+mac_folder+"/process-sortable.php?"+order+"&pica_token="+pica_token);
                    if(f){
     ...
     upload_url: site_url+"/wp-content/plugins/"+mac_folder+"/picaPhotosResize.php?albumId=<?php echo $_REQUEST['albid'] ?>",
                ...
                file_size_limit : 0,
                post_params: {"upload_token" : upload_token},
